...
import { AssetDocumentsPanel } from "@/features/assets/asset-documents-panel";
 ...
 type Props = { mode: Mode; assetId?: string };
...
async function resolveCategoryLabelById({
  organizationId,
  categoryId,
  signal,
}: {
  ...
  const { data } = await fetchCategoriesPage({ organizationId, limit: 30, offset: 0, signal });
  const found = data.find((d) => d.id === categoryId);
  return found?.label ?? null;
}

async function resolveLocationLabelById({
  organizationId,
  locationId,
  signal,
}: {
  ...
  const { data } = await fetchLocationsPage({ organizationId, limit: 200, offset: 0, signal });
  const found = data.find((d) => d.id === locationId);
  return found?.label ?? null;
}

 export function AssetForm({ mode, assetId }: Props) {
   ...
   const [qrCode, setQrCode] = useState<string>(mode === "create" ? "" : "");
   ...
   useEffect(() => {
    const ac = new AbortController();
     async function load() {
       ...
       try {
         const a = await apiFetch<PlenumAsset>(
           `/api/v1/plenum/assets/${encodeURIComponent(assetId)}`,
           { signal: ac.signal },
         );
   ...
   // Reset cat/location/qr when org changes
   useEffect(() => {
     setCategory(null);
     setLocationSel(null);
     if (mode === "create") setQrCode("");
   }, [organizationId, mode]);
 
  // Resolve labels for ids (edit prefill) so dropdown shows names not raw ids
  useEffect(() => {
    if (mode !== "edit") return;
    if (!organizationId) return;
    const ac = new AbortController();
    async function run() {
      if (category?.id && category.label === category.id) {
        const label = await resolveCategoryLabelById({
          organizationId,
          categoryId: category.id,
          signal: ac.signal,
        }).catch(() => null);
        if (label) setCategory({ id: category.id, label });
      }
      if (locationSel?.id && locationSel.label === locationSel.id) {
        const label = await resolveLocationLabelById({
          organizationId,
          locationId: locationSel.id,
          signal: ac.signal,
        }).catch(() => null);
        if (label) setLocationSel({ id: locationSel.id, label });
      }
    }
    void run();
    return () => ac.abort();
  }, [mode, organizationId, category?.id, category?.label, locationSel?.id, locationSel?.label]);
     ...
     <form
       ...
       onSubmit={async (e) => {
         ...
         try {
           ...
           router.push(mode === "create" ? APP_ROUTES.assets : `${APP_ROUTES.assets}/${assetId}`);
         } catch (err) {
         ...
         <CardContent className="space-y-6">
           ...
           <div className="flex items-center gap-2 justify-end">
             <Button asChild variant="secondary" disabled={pending}>
               <Link href={mode === "create" ? APP_ROUTES.assets : `${APP_ROUTES.assets}/${assetId}`}>Cancel</Link>
             </Button>
             <Button disabled={disableSubmit} type="submit">
               {pending ? (mode === "create" ? "Creating..." : "Saving...") : mode === "create" ? "Create Asset" : "Save"}
             </Button>
           </div>
         </CardContent>
       </Card>

      {mode === "edit" && assetId ? <AssetDocumentsPanel assetId={assetId} /> : null}
     </form>
   );
 }
